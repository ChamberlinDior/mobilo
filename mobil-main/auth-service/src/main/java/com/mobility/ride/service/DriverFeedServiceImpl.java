...
public class DriverFeedServiceImpl implements DriverFeedService {
    ...
    public List<RideOfferDto> findScheduledRides(Double lat, Double lng, Double radiusKm) {
        ...
        OffsetDateTime then = now.plusMinutes(30);
        return rideRepository.findScheduledBetween(now, then)
                .stream()
                .filter(r -> distance(lat, lng, r.getPickupLat(), r.getPickupLng()) <= radiusKm)
                .map(r -> mapToDto(r, lat, lng))
                .toList();
    }

    private RideOfferDto mapToDto(Ride r, double originLat, double originLng) {
        double dist = distance(originLat, originLng, r.getPickupLat(), r.getPickupLng());
        return RideOfferDto.builder()
                ...
                .scheduledAt(r.getScheduledAt() != null ? r.getScheduledAt().toString() : null)
                .distanceKm(dist)
                .weightKg(r.getWeightKg() != null ? r.getWeightKg().doubleValue() : null)
                .build();
    }
